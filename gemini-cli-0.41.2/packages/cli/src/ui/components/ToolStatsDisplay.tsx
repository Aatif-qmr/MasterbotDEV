...
export const ToolStatsDisplay: React.FC = () => {
  ...
  const totalDecisions = Object.values(tools.byName).reduce(
    (acc, tool) => {
      ...
      acc[ToolCallDecision.AUTO_ACCEPT] +=
        tool.decisions[ToolCallDecision.AUTO_ACCEPT] ?? 0;
      return acc;
    },
    { accept: 0, reject: 0, modify: 0, [ToolCallDecision.AUTO_ACCEPT]: 0 },
  );
  ...
  const totalReviewed =
    ...
    totalDecisions[ToolCallDecision.AUTO_ACCEPT];
  const agreementRate =
    ...
    >
      ...
      <Box>
        <Box
          width={TOOL_NAME_COL_WIDTH + CALLS_COL_WIDTH + SUCCESS_RATE_COL_WIDTH}
        >
          <Text color={theme.text.primary}> » Auto-Accepted:</Text>
        </Box>
        <Box width={AVG_DURATION_COL_WIDTH} justifyContent="flex-end}>
