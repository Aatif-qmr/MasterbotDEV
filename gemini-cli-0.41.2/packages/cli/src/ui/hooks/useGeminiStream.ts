...
export const useGeminiStream = (
  ...
  // If the stream is not enabled, return an inert state
  if (!enabled) {
    // This return value must match the structure of the actual return value below
    return {
      streamingState: StreamingState.Idle,
      submitQuery: async () => {}, // No-op
      initError: null,
      pendingHistoryItems: [],
      thought: null,
      cancelOngoingRequest: () => {}, // No-op
      pendingToolCalls: [],
      handleApprovalModeChange: async () => {}, // No-op
      activePtyId: undefined,
      loopDetectionConfirmationRequest: null,
      lastOutputTime: 0,
      backgroundTaskCount: 0,
      isBackgroundTaskVisible: false,
      toggleBackgroundTasks: () => {}, // No-op
      backgroundCurrentExecution: () => {}, // No-op
      backgroundTasks: new Map(),
      dismissBackgroundTask: () => {}, // No-op
      retryStatus: null,
    };
  }
