...
export class Config implements McpContext, AgentLoopContext {
  ...
  async refreshAuth(
    ...
    let codeAssistServer: CodeAssistServer | undefined;
    if (
      newContentGeneratorConfig.authType === AuthType.LOGIN_WITH_GOOGLE ||
      newContentGeneratorConfig.authType === AuthType.COMPUTE_ADC ||
      newContentGeneratorConfig.authType === AuthType.AUTO ||
      newContentGeneratorConfig.authType === AuthType.NONE
    ) {
      const httpOptions = { headers: newContentGeneratorConfig.customHeaders };
      codeAssistServer = await createCodeAssistServer(
        httpOptions,
    ...
    if (codeAssistServer) {
      this.contentGenerator = new LoggingContentGenerator(codeAssistServer, this);
    } else {
  ...
  activateFallbackMode(targetModel: string): void {
    this.setModel(targetModel, true);
    const authType = this.getContentGeneratorConfig()?.authType;
    if (authType) {
      logFlashFallback(this, new FlashFallbackEvent(authType, targetModel));
    }
  }
