...
export async function scheduleAgentTools(
  ...
  const agentConfigForSchedulerContext: Config = {
    ...config,
    toolRegistry,
    getPromptRegistry: () => promptRegistry ?? config.getPromptRegistry(),
    getResourceRegistry: () => resourceRegistry ?? config.getResourceRegistry(),
    promptId: config.promptId,
  };

  const schedulerContext = {
    config: agentConfigForSchedulerContext,
    promptId: config.promptId,
    toolRegistry: agentConfigForSchedulerContext.toolRegistry,
    promptRegistry: agentConfigForSchedulerContext.getPromptRegistry(),
    resourceRegistry: agentConfigForSchedulerContext.getResourceRegistry(),
    messageBus: agentConfigForSchedulerContext.toolRegistry.messageBus,
