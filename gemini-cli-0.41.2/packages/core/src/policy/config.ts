      ...
      if (message.persist) {
        persistenceQueue = persistenceQueue.then(async () => {
          try {
            ...
            let canonicalArgsPattern: string | undefined = undefined;

            if (message.commandPrefix) {
              const patterns = buildArgsPatterns(undefined, message.commandPrefix);
              if (patterns.length > 0) {
                // For persistence, we will take the first pattern.
                // The in-memory rules will have all patterns added.
                canonicalArgsPattern = patterns[0];
              }
            } else if (message.argsPattern) {
              canonicalArgsPattern = message.argsPattern;
            }

            // Create a canonical rule for comparison
            const newCanonicalRule: TomlRule = {
              toolName: normalizedToolName,
              mcpName: message.mcpName,
              argsPattern: canonicalArgsPattern,
            };

            // Filter out existing rules that are conceptually the same as the new rule
            const filteredRules: TomlRule[] = [];
            let existingRuleFoundAndMerged = false;

            for (const existingRule of existingData.rule) {
              // Check if the existing rule matches the canonical identifiers
              const isConceptualDuplicate =
                existingRule.toolName === newCanonicalRule.toolName &&
                existingRule.mcpName === newCanonicalRule.mcpName &&
                existingRule.argsPattern === newCanonicalRule.argsPattern;

              if (isConceptualDuplicate) {
                // Merge modes
                ...
                // Create an updated rule with merged modes and other properties from the new message
                const updatedRule: TomlRule = {
                  ...existingRule, // Keep existing properties
                  decision: 'allow', // Always 'allow' for auto-saved policies
                  priority: getAlwaysAllowPriorityFraction(),
                  allowRedirection:
                    message.allowRedirection !== undefined
                      ? message.allowRedirection
                      : existingRule.allowRedirection, // Preserve if not explicitly changed
                  modes: mergedModes.length > 0 ? mergedModes : undefined,
                };
                filteredRules.push(updatedRule);
                existingRuleFoundAndMerged = true;
              } else {
                filteredRules.push(existingRule);
              }
            }

            // If no existing rule was found and merged, add the new rule
            if (!existingRuleFoundAndMerged) {
              filteredRules.push(
                createTomlRule(normalizedToolName, message),
              );
            }

            existingData.rule = filteredRules;
