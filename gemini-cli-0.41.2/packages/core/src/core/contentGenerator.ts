...
import { createCodeAssistServer } from '../code_assist/codeAssist.js';
...
import { InstallationManager } from '../telemetry/installationManager.js';
...
export async function createContentGenerator(
  ...
  const generator = await (async () => {
    ...
    if (
      ...
      config.authType === AuthType.AUTO ||
      config.authType === AuthType.NONE
    ) {
      const httpOptions = { headers: baseHeaders };
      const codeAssistServer = await createCodeAssistServer(
        httpOptions,
        config.authType,
        gcConfig,
        sessionId,
      ...
      if (codeAssistServer) {
        return new LoggingContentGenerator(codeAssistServer, gcConfig);
      } else {
        // Fallback to FakeContentGenerator if no code assist server is created
        // This will allow us to create "zero auth" experiences
        const fakeGenerator = await FakeContentGenerator.fromFile(
          gcConfig.fakeResponses || '',
        );
        return new LoggingContentGenerator(fakeGenerator, gcConfig);
