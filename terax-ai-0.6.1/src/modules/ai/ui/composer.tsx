...
import { expandSnippetTokens, type Snippet } from "../engine/snippets";
import { tryRunSlashCommand, type SlashCommandMeta } from "../engine/slashCommands";
...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("cipher:ai-attach-file", onAttach);
    return () => window.removeEventListener("cipher:ai-attach-file", onAttach);
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<cipher-command name="${outcome.commandName}" />`;
        }
      }
    }
